...
## The command is `copilot-session-tui`, never `cst`

**`cst` will not work for you.** It is a shell function the installer writes into a
human's shell profile, and the shell you run commands in does not load that profile. You
will get "command not found" and conclude the feature is unavailable. It is not; you are
calling it by a name that only exists in somebody else's shell.

The program is `copilot-session-tui`, on `PATH`. If it is somehow not, the environment
variable `CST_BIN` holds the absolute path to the exact CST running this session.

## Post through `copilot-session-tui thread`, not through `gh`

```bash
copilot-session-tui thread post <url> --body-file - <<'EOF'
Your message here.
EOF
```

`thread post` comments **and** records the comment id as yours. `gh issue comment` writes
the same comment but records nothing, and CST will then read your own words as a reply
worth waking you for. You will wake yourself up, read your own question, and have nothing
to do.

Use `--body-file -` and a heredoc for anything longer than a sentence. It avoids shell
quoting mangling the message.

The URL carries the repository, so a thread in another repository works exactly the same
and you do not need to be anywhere near it.

Other commands:

- `copilot-session-tui thread watch <url>` — start being woken by a thread you did not
  post to. Use after creating an item with `gh issue create`, since creating is not
  posting.
- `copilot-session-tui thread list` — what you are watching, and anything that could not
  be delivered.
- `copilot-session-tui thread leave <url>` — stop being woken. Other agents watching it
  are unaffected.
- `copilot-session-tui thread close <url>` — end the correspondence for everyone. The
  GitHub item itself stays open; closing that is a human's call.
...
If it turns out the thread no longer concerns you, run
`copilot-session-tui thread leave <url>`. Doing that is not giving up; it is the
difference between a channel that stays useful and one everybody learns to ignore.
...
**If it is answered and you are done, leave the thread.** One you no longer need to hear
about is one you should stop being woken by:
`copilot-session-tui thread leave <url>`.
