...
pub(crate) struct ViewState {
    ...
    /// Preferred screen column retained while moving through wrapped visual rows.
    pub(crate) vertical_goal_col: Option<usize>,
...
impl ViewState {
    ...
    fn scroll_down(
        ...
        for (i, line) in lines.iter_row().rev().skip(skip).enumerate() {
            let current_row_height = LineWrapper::wrap_line(line, max_width, self.tab_width)
                .len()
                .max(1);
