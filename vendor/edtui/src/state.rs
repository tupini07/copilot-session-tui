...
impl EditorState {
    ...
    /// Clears the retained screen column used by wrapped Up/Down movement.
    ///
    /// Call this before edits or non-vertical movement so the next Up/Down starts
    /// from the cursor's current visual column.
    pub fn reset_vertical_goal(&mut self) {
        self.view.vertical_goal_col = None;
    }
